...
import {
  Shield, Users, MessageSquare, FileText, AlertTriangle, Ban, Trash2,
  Star, UserPlus, UserX, Search, X, Eye, Activity, Clock, Check, ChevronLeft, ChevronRight,
} from "lucide-react"
import { useTranslation } from "react-i18next"

interface Stats {
  users: number; conversations: number; messages: number; reports: number; bans: number
  registrationsToday: number; onlineUsers: number; messagesToday: number
}

interface AdminUser {
  id: string; username: string; email: string; displayName: string | null
  status: string; avatar?: string | null; createdAt: string
}

interface AdminReport {
  id: string; reportedBy: string; targetUserId: string | null; targetMessageId: string | null
  reason: string; status: string; createdAt: string
  reportedByName?: string; targetUserName?: string | null
}

interface AdminBan {
  id: string; conversationId: string; userId: string; bannedBy: string
  reason: string | null; expiresAt: string | null; createdAt: string
}

interface ActivityItem {
  id: string; type: string; userId: string; username: string; content: string; createdAt: string
}
...
export function AdminPage() {
  ...
  const loadUsers = useCallback(async () => {
    ...
    if (data) { setUserList(data.users); setUserTotal(data.total) }
  ...
  const loadReports = useCallback(async () => {
    ...
    if (data) { setReportList(data.reports); setReportTotal(data.total) }
  ...
  const loadBans = useCallback(async () => {
    ...
    if (data) { setBanList(data.bans); setBanTotal(data.total) }
  }, [banQuery, banPage])
  ...
  const loadAdmins = useCallback(async () => {
    ...
    if (data) { setOwnerId(data.ownerId); setAdminIds(data.adminIds) }
  }, [])

  const searchUsers = () => { setUserPage(1); loadUsers() }
  ...
  const toggleSuspend = async (id: string, currentlySuspended: boolean) => {
    await api(`/api/admin/users/${id}/suspend`, {
      method: "PUT", body: JSON.stringify({ suspended: !currentlySuspended }),
    }).catch(() => {})
    setUserList((prev) => prev.map((u) => u.id === id ? { ...u, status: currentlySuspended ? "offline" : "busy" } : u))
  }
  ...
  const addAdmin = async () => {
    ...
    setAdminMsg("")
    try {
      const res = await api<{ adminIds: string[] }>("/api/admin/admins", { method: "POST", body: JSON.stringify({ userId: addAdminId.trim() }) })
      setAdminIds(res.adminIds); setAddAdminId(""); setAdminMsg(t("admin.adminAdded"))
    } catch { setAdminMsg(t("admin.failed")) }
  }
  ...
  const removeAdmin = async (userId: string) => {
    ...
    try {
      ...
      setAdminIds(res.adminIds); setAdminMsg(t("admin.adminRemoved"))
    } catch { setAdminMsg(t("admin.failed")) }
  }
    ...
    <div className="flex h-full">
      ...
      <div className="flex-1 overflow-y-auto p-6">
            ...
            {stats ? (
              <>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-6">
                  <StatCard icon={Users} label={t("admin.users")} value={stats.users} sub={t("admin.onlineUsers") + ": " + stats.onlineUsers} />
                  <StatCard icon={MessageSquare} label={t("admin.messages")} value={stats.messages} sub={t("admin.today") + ": " + stats.messagesToday} />
                  <StatCard icon={FileText} label={t("admin.conversations")} value={stats.conversations} />
                  <StatCard icon={AlertTriangle} label={t("admin.reports")} value={stats.reports} sub={t("admin.open") + ": " + (stats.reports)} />
        ...
        {tab === "users" && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-lg font-semibold text-text-primary">{t("admin.users")} ({userTotal})</h1>
            </div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <button onClick={searchUsers} className="h-9 px-4 rounded-2xl bg-accent text-white text-sm font-medium hover:bg-accent-hover transition-all cursor-pointer">
                {t("common.search")}
              </button>
            </div>
              ...
              {userList.map((u) => (
                <div key={u.id} className="flex items-center gap-3 rounded-2xl border border-border bg-surface px-4 py-3">
                  ...
                  <span className={`text-xs capitalize px-2.5 py-1 rounded-xl ${
                    ...
                    u.status === "away" ? "bg-yellow-500/10 text-yellow-400" :
                    u.status === "busy" ? "bg-red-500/10 text-red-400" :
                    "bg-text-muted/10 text-text-muted"
                  }`}>{u.status}</span>
                  <button onClick={() => viewUserDetail(u.id)} className="p-1.5 rounded-xl text-text-muted hover:text-text-primary hover:bg-white/5 cursor-pointer" title={t("admin.viewDetails")}><Eye className="h-3.5 w-3.5" /></button>
                  <button onClick={() => toggleSuspend(u.id, u.status === "busy")} className={`p-1.5 rounded-xl cursor-pointer ${u.status === "busy" ? "text-green-500 hover:text-green-400" : "text-text-muted hover:text-yellow-400"}`} title={u.status === "busy" ? t("admin.unsuspend") : t("admin.suspend")}>
                    {u.status === "busy" ? <Check className="h-3.5 w-3.5" /> : <Ban className="h-3.5 w-3.5" />}
                  </button>
                  <button onClick={() => deleteUser(u.id)} className="p-1.5 rounded-xl text-text-muted hover:text-danger cursor-pointer" title={t("admin.deleteUser")}><Trash2 className="h-3.5 w-3.5" /></button>
                </div>
              ))}
              {userList.length === 0 && <p className="text-sm text-text-muted text-center py-4">{t("common.noResults")}</p>}
            </div>
            {userTotal > 50 && (
              <div className="flex items-center justify-center gap-3 mt-4">
                <button disabled={userPage <= 1} onClick={() => setUserPage((p) => p - 1)} className="flex items-center gap-1 h-8 px-3 rounded-xl border border-border text-sm text-text-muted hover:text-text-primary disabled:opacity-30 cursor-pointer disabled:cursor-default"><ChevronLeft className="h-3.5 w-3.5" /> {t("common.prev")}</button>
                <span className="text-xs text-text-muted">{userPage} / {Math.ceil(userTotal / 50)}</span>
                <button disabled={userPage >= Math.ceil(userTotal / 50)} onClick={() => setUserPage((p) => p + 1)} className="flex items-center gap-1 h-8 px-3 rounded-xl border border-border text-sm text-text-muted hover:text-text-primary disabled:opacity-30 cursor-pointer disabled:cursor-default">{t("common.next")} <ChevronRight className="h-3.5 w-3.5" /></button>
              </div>
            )}
          </div>
        )}
        ...
        {tab === "reports" && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-lg font-semibold text-text-primary">{t("admin.reports")} ({reportTotal})</h1>
              <select value={reportStatusFilter} onChange={(e) => { setReportStatusFilter(e.target.value); setReportPage(1) }}
                className="h-9 rounded-2xl border border-border bg-surface px-3 text-sm text-text-primary outline-none focus:border-accent/50">
                <option value="">{t("admin.allStatuses")}</option>
            ...
            {reportTotal > 50 && (
              <div className="flex items-center justify-center gap-3 mt-4">
                <button disabled={reportPage <= 1} onClick={() => setReportPage((p) => p - 1)} className="flex items-center gap-1 h-8 px-3 rounded-xl border border-border text-sm text-text-muted hover:text-text-primary disabled:opacity-30 cursor-pointer disabled:cursor-default"><ChevronLeft className="h-3.5 w-3.5" /> {t("common.prev")}</button>
                <span className="text-xs text-text-muted">{reportPage} / {Math.ceil(reportTotal / 50)}</span>
                <button disabled={reportPage >= Math.ceil(reportTotal / 50)} onClick={() => setReportPage((p) => p + 1)} className="flex items-center gap-1 h-8 px-3 rounded-xl border border-border text-sm text-text-muted hover:text-text-primary disabled:opacity-30 cursor-pointer disabled:cursor-default">{t("common.next")} <ChevronRight className="h-3.5 w-3.5" /></button>
              </div>
            )}
          </div>
        )}
        ...
        {tab === "bans" && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-lg font-semibold text-text-primary">{t("admin.bans")} ({banTotal})</h1>
              <div className="relative max-w-xs">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-text-muted" />
                <input value={banQuery} onChange={(e) => setBanQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && loadBans()}
            ...
            <div className="space-y-2">
              {banList.map((b) => (
                <div key={b.id} className="flex items-center gap-3 rounded-2xl border border-border bg-surface px-4 py-3">
                  ...
                  <button onClick={() => removeBan(b.id)} className="text-xs text-accent hover:text-accent-hover cursor-pointer">{t("admin.unban")}</button>
                </div>
              ))}
            ...
            {banTotal > 50 && (
              <div className="flex items-center justify-center gap-3 mt-4">
                <button disabled={banPage <= 1} onClick={() => setBanPage((p) => p - 1)} className="flex items-center gap-1 h-8 px-3 rounded-xl border border-border text-sm text-text-muted hover:text-text-primary disabled:opacity-30 cursor-pointer disabled:cursor-default"><ChevronLeft className="h-3.5 w-3.5" /> {t("common.prev")}</button>
                <span className="text-xs text-text-muted">{banPage} / {Math.ceil(banTotal / 50)}</span>
                <button disabled={banPage >= Math.ceil(banTotal / 50)} onClick={() => setBanPage((p) => p + 1)} className="flex items-center gap-1 h-8 px-3 rounded-xl border border-border text-sm text-text-muted hover:text-text-primary disabled:opacity-30 cursor-pointer disabled:cursor-default">{t("common.next")} <ChevronRight className="h-3.5 w-3.5" /></button>
              </div>
            )}
          </div>
        )}
        ...
        {tab === "activity" && (
          <div>
            ...
            <div className="space-y-1.5">
              {activityList.map((a) => (
                <div key={a.id + a.type} className="flex items-center gap-3 rounded-2xl border border-border bg-surface px-4 py-2.5">
                  <div className={`flex h-7 w-7 items-center justify-center rounded-full shrink-0 ${
                    a.type === "message" ? "bg-accent/10 text-accent" :
                    a.type === "report" ? "bg-yellow-500/10 text-yellow-400" :
                    "bg-danger/10 text-danger"
                  }`}>
                    {a.type === "message" ? <MessageSquare className="h-3.5 w-3.5" /> :
                     a.type === "report" ? <AlertTriangle className="h-3.5 w-3.5" /> :
                     <Ban className="h-3.5 w-3.5" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-text-primary truncate">
                      <span className="font-medium">{a.username}</span>
                      {a.type === "message" ? ` sent a message` :
              ...
              {activityList.length === 0 && <p className="text-sm text-text-muted text-center py-4">{t("common.noResults")}</p>}
            </div>
          </div>
        )}
        ...
        {tab === "admins" && (
          <div>
            ...
            <div className="rounded-2xl border border-border bg-surface p-4 mb-4">
              ...
              <p className="text-sm text-text-primary font-mono">{ownerId ? `${ownerId.slice(0, 8)}...` : t("admin.notSet")}</p>
            </div>
            ...
            <div className="space-y-2 mb-4">
              ...
              {adminIds.map((id) => (
                <div key={id} className="flex items-center justify-between rounded-2xl border border-border bg-surface px-4 py-3">
                  ...
                  <button onClick={() => removeAdmin(id)} className="flex items-center gap-1 text-xs text-danger hover:text-danger/80 cursor-pointer">
            ...
            <div className="rounded-2xl border border-border bg-surface p-4">
              ...
              <div className="flex gap-2">
                ...
                />
                <button onClick={addAdmin} disabled={!addAdminId.trim()}
      ...
      {detailUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setDetailUser(null)}>
          <div className="w-full max-w-lg rounded-[32px] border border-border bg-surface p-6 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-text-primary">{t("admin.userDetails")}</h3>
              <button onClick={() => setDetailUser(null)} className="text-text-muted hover:text-text-primary cursor-pointer"><X className="h-4 w-4" /></button>
            </div>
            {detailLoading ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-text-primary">{detailUser.displayName || detailUser.username}</p>
                    <p className="text-xs text-text-muted">@{detailUser.username} · {detailUser.email}</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="rounded-2xl bg-bg-primary px-3 py-2">
                    ...
                    <p className="text-sm font-medium text-text-primary">{detailUser.isAdmin ? t("admin.admin") : t("admin.member")}</p>
                  </div>
                </div>
                {detailUser.createdAt && (
                  <p className="text-xs text-text-muted">{t("admin.memberSince")}: {new Date(detailUser.createdAt).toLocaleDateString()}</p>
                )}
