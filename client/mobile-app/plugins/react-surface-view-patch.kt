...
public class ReactSurfaceView(context: Context?, private val surface: ReactSurfaceImpl) :
  ...
  override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
    ...
    // Patch: force the measure specs to the full window size. On some devices a
    // stuck IME insets animation measures the surface with the visible-frame
    // height (full window minus phantom keyboard insets), permanently shrinking
    // the React root view to a fraction of the window. The display metrics
    // always reflect the full screen, so the root can never be sized smaller.
    val dm = context?.resources?.displayMetrics
    val fullWidthSpec =
        if (dm != null) MeasureSpec.makeMeasureSpec(dm.widthPixels, MeasureSpec.EXACTLY)
        else widthMeasureSpec
    val fullHeightSpec =
        if (dm != null) MeasureSpec.makeMeasureSpec(dm.heightPixels, MeasureSpec.EXACTLY)
        else heightMeasureSpec
    var width = 0
    var height = 0
    val widthMode = MeasureSpec.getMode(fullWidthSpec)
    if (widthMode == MeasureSpec.AT_MOST || widthMode == MeasureSpec.UNSPECIFIED) {
      ...
      width = MeasureSpec.getSize(fullWidthSpec)
    }
    val heightMode = MeasureSpec.getMode(fullHeightSpec)
    if (heightMode == MeasureSpec.AT_MOST || heightMode == MeasureSpec.UNSPECIFIED) {
      ...
      height = MeasureSpec.getSize(fullHeightSpec)
    }
    setMeasuredDimension(width, height)
    wasMeasured = true
    this.widthMeasureSpec = fullWidthSpec
    this.heightMeasureSpec = fullHeightSpec
    val viewportOffset = viewportOffset
    surface.updateLayoutSpecs(
        fullWidthSpec, fullHeightSpec, viewportOffset.x, viewportOffset.y)
