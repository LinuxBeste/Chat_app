        ...
        <View style={s.infoPanel}>
          <View style={s.infoHeader}>
            <Text style={s.infoTitle}>
              {isDm ? (otherMember?.displayName ?? otherMember?.username ?? "Details") : convInfo?.name || "Details"}
            </Text>
            ...
            {isDm && otherMember && (
              <>
                {otherMember.avatar ? (
                  ...
                  <View style={s.infoAvatar}>
                    <Text style={s.infoAvatarText}>
                      {(otherMember.displayName ?? otherMember.username)[0].toUpperCase()}
                    </Text>
                  </View>
                )}
                ...
                <TouchableOpacity style={s.infoAction} onPress={() => Clipboard.setStringAsync(otherMember.id)}>
                  <Copy size={16} color="#6C8CFF" />
                  <Text style={s.infoActionText}> Copy User ID</Text>
                </TouchableOpacity>
                <View style={s.infoDivider} />
                {friendStatus !== "self" && (
                  <TouchableOpacity
                    style={s.infoAction}
                    onPress={friendStatus === "none" ? addFriend : undefined}
                ...
                <TouchableOpacity style={s.infoAction} onPress={toggleBlock}>
                  ...
                  <Text style={[s.infoActionText, { color: "#EF4444" }]}>
                    {isBlocked ? " Unblock User" : " Block User"}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity style={s.infoAction} onPress={reportUser}>
                  <Flag size={16} color="#EF4444" />
                  <Text style={[s.infoActionText, { color: "#EF4444" }]}> Report User</Text>
                </TouchableOpacity>
                <View style={s.infoDivider} />
                <TouchableOpacity
                  style={s.infoAction}
                ...
                >
                  ...
                  <Text style={[s.infoActionText, { color: "#EF4444" }]}> Delete Conversation</Text>
                </TouchableOpacity>
              </>
            )}
            {!isDm && convInfo && (
              <>
                ...
                {renaming ? (
                  <View style={s.renameRow}>
                    <TextInput
                      ...
                      autoFocus
                    />
                    <TouchableOpacity style={s.renameSave} onPress={renameGroup}>
                  ...
                  >
                    ...
                    <Text style={s.infoActionText}> Rename Group</Text>
                  </TouchableOpacity>
                )}
                <View style={s.infoDivider} />
                ...
                <TouchableOpacity style={s.infoAction} onPress={toggleMute}>
                  ...
                  <Text style={[s.infoActionText, { color: muted ? "#EF4444" : "#8888A0" }]}>
                    {muted ? " Unmute" : " Mute"}
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity style={s.infoAction} onPress={() => setShowAddPeople(true)}>
                  <UserPlus size={16} color="#6C8CFF" />
                  <Text style={s.infoActionText}> Add People</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={s.infoAction}
                  onPress={() => {
                ...
                >
                  ...
                  <Text style={s.infoActionText}> Search in Conversation</Text>
                </TouchableOpacity>
                <View style={s.infoDivider} />
                <TouchableOpacity style={s.infoAction} onPress={leaveGroup}>
                  <LogOut size={16} color="#EF4444" />
                  <Text style={[s.infoActionText, { color: "#EF4444" }]}> Leave Group</Text>
                </TouchableOpacity>
              </>
            )}
          </ScrollView>
      ...
      <View style={[s.header, { paddingTop: insets.top + 10 }]}>
        ...
        <TouchableOpacity style={s.headerCenter} onPress={() => setShowInfo(true)}>
          <Text style={s.title} numberOfLines={1}>
            {isDm ? (otherMember?.displayName ?? otherMember?.username ?? "User") : convInfo?.name || "Group"}
          </Text>
          <Text style={s.subtitle}>
            {typingUsers.length > 0
              ? "typing..."
              : convInfo?.members?.length
            ...
            <ScrollView contentContainerStyle={s.previewScrollContent}>
              {previewFile.messageType === "image" || previewFile.fileType?.startsWith("image/") ? (
                ...
                <View style={s.previewFile}>
                  ...
                  <Text style={s.previewFileText}>
                    {" "}
                    {previewFile.fileName || previewFile.attachment?.filename || "File"}
                  </Text>
                  {(previewFile.fileUrl || previewFile.attachment?.url) && (
                    <TouchableOpacity
                      style={s.downloadBtn}
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  ...
                  <Text style={{ color: "#E8E8F0", fontSize: 14 }} numberOfLines={2}>
                    {item.content}
                  </Text>
                  <Text style={{ color: "#585870", fontSize: 11, marginTop: 2 }}>
              ...
              ListEmptyComponent={
                searchQuery.length >= 2 ? (
                  <Text style={{ color: "#585870", textAlign: "center", marginTop: 40, fontSize: 14 }}>
                    No messages found
                  </Text>
                ) : null
              }
      ...
      {editingId ? (
        <View style={s.editInputRow}>
          <TextInput
            ...
            autoFocus
          />
          <TouchableOpacity style={s.saveBtn} onPress={confirmEdit}>
        ...
        <View style={s.inputRow}>
          ...
          <TouchableOpacity style={s.attachBtn} onPress={() => setShowEmoji(true)}>
            <SmilePlus size={20} color="#8888A0" />
          </TouchableOpacity>
          <TextInput
            style={s.input}
