...
import { View, Text, FlatList, TouchableOpacity, StyleSheet, RefreshControl, TextInput, Alert, Modal, ActivityIndicator } from "react-native"
  ...
  const deleteGroup = (id: string) => {
    Alert.alert(t("groups.deleteGroup"), "", [
      ...
      { text: t("common.delete"), style: "destructive", onPress: () => {
        api(`/api/conversations/${id}`, { method: "DELETE" }).then(load).catch(() => {})
      }},
    ])
  }
  ...
  return (
    <View style={s.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={s.item} onPress={() => onSelectChat?.(item.id)} onLongPress={() => deleteGroup(item.id)}>
            <View style={s.avatar}><Text style={s.avatarText}>{item.name?.[0] ?? "G"}</Text></View>
      ...
      <Modal visible={modalVisible} transparent animationType="slide" onRequestClose={() => setModalVisible(false)}>
        <View style={s.overlay}><View style={s.modal}>
          <View style={s.modalHeader}>
            ...
            <TouchableOpacity onPress={() => { setModalVisible(false); setSelectedParticipants(new Set()); setSearchQuery("") }}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity style={s.userItem} onPress={() => toggleParticipant(item.id)}>
                <View style={s.userAvatar}><Text style={s.userAvatarText}>{item.username[0].toUpperCase()}</Text></View>
                <View style={{ flex: 1 }}>
            ...
            ListEmptyComponent={searchQuery.length >= 2 && !searching ? <Text style={s.empty}>No users found</Text> : null}
          />
          <View style={s.modalActions}>
            <TouchableOpacity onPress={() => { setModalVisible(false); setSelectedParticipants(new Set()); setSearchQuery("") }} style={s.cancelBtn}>
              <Text style={s.cancelText}>{t("common.cancel")}</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={createGroup} style={[s.confirmBtn, !groupName.trim() && { opacity: 0.4 }]} disabled={!groupName.trim()}>
        ...
        <View style={s.overlay}><View style={s.modal}>
          ...
          <View style={s.modalActions}>
            ...
            <TouchableOpacity onPress={async () => {
              ...
              try {
                await api("/api/conversations/join", { method: "POST", body: JSON.stringify({ code: inviteCode.trim() }) })
                setJoinModal(false); setInviteCode(""); load()
              } catch {}
            }} style={s.confirmBtn}>
...
const s = StyleSheet.create({
  ...
  avatar: { width: 48, height: 48, borderRadius: 24, backgroundColor: "#181825", justifyContent: "center", alignItems: "center", marginRight: 14, borderWidth: 1, borderColor: "#252538" },
  ...
  modal: { backgroundColor: "#0A0A0F", borderTopLeftRadius: 20, borderTopRightRadius: 20, maxHeight: "90%", paddingBottom: 40, borderTopWidth: 1, borderTopColor: "#252538", paddingHorizontal: 20 },
  ...
  modalInput: { backgroundColor: "#101016", borderRadius: 12, padding: 14, color: "#E8E8F0", fontSize: 15, borderWidth: 1, borderColor: "#252538", marginBottom: 12 },
  searchRow: { flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: "#101016", borderRadius: 12, paddingHorizontal: 12, paddingVertical: 10, borderWidth: 1, borderColor: "#252538", marginBottom: 8 },
  searchInput: { flex: 1, color: "#E8E8F0", fontSize: 14, padding: 0 },
  chipRow: { flexDirection: "row", flexWrap: "wrap", gap: 6, marginBottom: 8 },
  chip: { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#6C8CFF", borderRadius: 16, paddingHorizontal: 10, paddingVertical: 4 },
  chipText: { color: "#FFFFFF", fontSize: 12, fontWeight: "500" },
  userItem: { flexDirection: "row", alignItems: "center", paddingVertical: 10, gap: 10 },
  userAvatar: { width: 36, height: 36, borderRadius: 18, backgroundColor: "#181825", justifyContent: "center", alignItems: "center", borderWidth: 1, borderColor: "#252538" },
