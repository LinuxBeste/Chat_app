  ...
  return (
    <View style={[s.container, { backgroundColor: c.bg }]}>
      <View style={[s.header, { paddingTop: insets.top + 12, borderBottomColor: c.borderLight }]}>
        <Text style={[s.title, { color: c.text }]}>{t("groups.title")}</Text>
        <View style={{ flexDirection: "row", gap: 8 }}>
          <TouchableOpacity style={[s.createBtn, { backgroundColor: c.accent }]} onPress={() => setModalVisible(true)}>
            <Text style={s.createBtnText}>+ {t("groups.newGroup")}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[s.createBtn, { backgroundColor: c.accent }]} onPress={() => setJoinModal(true)}>
        ...
        renderItem={({ item }) => (
          ...
          >
            <View style={[s.avatar, { backgroundColor: c.surfaceAlt, borderColor: c.border }]}>
              <Text style={[s.avatarText, { color: c.text }]}>{item.name?.[0] ?? "G"}</Text>
            </View>
            <View style={s.itemContent}>
              <Text style={[s.name, { color: c.text }]}>{item.name ?? "Unnamed"}</Text>
              {item.participantCount !== undefined && (
                <Text style={[s.meta, { color: c.textMuted }]}>{item.participantCount} members</Text>
              )}
            </View>
          </TouchableOpacity>
        )}
        ListEmptyComponent={<Text style={[s.empty, { color: c.textMuted }]}>{t("groups.noGroups")}</Text>}
      />
      <Modal visible={modalVisible} transparent animationType="slide" onRequestClose={() => setModalVisible(false)}>
        <View style={s.overlay}>
          <View style={[s.modal, { backgroundColor: c.sheetBg, borderTopColor: c.border }]}>
              ...
              <View style={s.chipRow}>
                {Array.from(selectedParticipants).map((id) => {
                  ...
                  return (
                    <TouchableOpacity
                      key={id}
                      style={[s.chip, { backgroundColor: c.accent }]}
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity style={s.userItem} onPress={() => toggleParticipant(item.id)}>
                  ...
                  <View style={{ flex: 1 }}>
                    <Text style={[s.userName, { color: c.text }]}>{item.displayName || item.username}</Text>
                    <Text style={[s.userTag, { color: c.textMuted }]}>@{item.username}</Text>
                  </View>
            ...
            <View style={s.modalActions}>
              ...
              <TouchableOpacity
                onPress={createGroup}
                style={[s.confirmBtn, { backgroundColor: c.accent }, !groupName.trim() && { opacity: 0.4 }]}
      ...
      <Modal visible={joinModal} transparent animationType="fade" onRequestClose={() => setJoinModal(false)}>
        <View style={s.overlay}>
          <View style={[s.modal, { backgroundColor: c.sheetBg, borderTopColor: c.border }]}>
            <Text style={[s.modalTitle, { color: c.text }]}>Join Group</Text>
            <TextInput
              style={[s.modalInput, { backgroundColor: c.inputBg, color: c.text, borderColor: c.border }]}
...
const s = StyleSheet.create({
  ...
  createBtn: { borderRadius: 20, paddingHorizontal: 16, paddingVertical: 8 },
