...
import { View, Text, FlatList, TouchableOpacity, StyleSheet, RefreshControl, TextInput, Modal, Alert, ActivityIndicator, Platform } from "react-native"
...
export function EventsScreen({ onSelectChat }: { onSelectChat?: (id: string) => void }) {
  ...
  useEffect(() => {
    api<Conv[]>("/api/conversations").then(setConvs).catch(() => {})
  }, [])
  ...
  const create = async () => {
    ...
    try {
      await api("/api/events", { method: "POST", body: JSON.stringify({
        ...
        startsAt: startsAt.toISOString(), endsAt: endsAt?.toISOString() || undefined,
      })})
      setTitle(""); setDesc(""); setStartsAt(new Date()); setEndsAt(null); setConvId("")
  ...
  const deleteEvent = (eventId: string) => {
    Alert.alert("Delete Event", "Are you sure?", [
      ...
      { text: "Delete", style: "destructive", onPress: () => {
        api(`/api/events/${eventId}`, { method: "DELETE" }).then(load).catch(() => {})
      }},
    ])
  }
  ...
  const formatDate = (iso: string) => {
    ...
    const day = isToday ? "Today" : isTomorrow ? "Tomorrow" : d.toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" })
  ...
  return (
    <View style={[st.container, { backgroundColor: c.bg }]}>
      ...
      <View style={[st.filterRow, { borderBottomColor: c.borderLight }]}>
        <TouchableOpacity style={[st.filterBtn, { backgroundColor: filter === "upcoming" ? c.accent : c.surfaceAlt }]} onPress={() => setFilter("upcoming")}>
          <Text style={[st.filterText, { color: filter === "upcoming" ? "#FFFFFF" : c.textSecondary }]}>Upcoming</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[st.filterBtn, { backgroundColor: filter === "past" ? c.accent : c.surfaceAlt }]} onPress={() => setFilter("past")}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            ...
            >
              ...
              {item.description ? (
                <Text style={[st.eventDesc, { color: c.textSecondary }]} numberOfLines={2}>{item.description}</Text>
              ) : null}
              {item.rsvpCount && (
              ...
              {filter === "upcoming" && (
                <View style={st.rsvpRow}>
                  {(["going", "maybe", "declined"] as const).map((status) => (
                    <TouchableOpacity
                      key={status}
                      style={[st.rsvpBtn, { borderColor: c.border, backgroundColor: item.myRsvp === status ? c.accentLight : c.surfaceAlt }, item.myRsvp === status && { borderColor: c.accent }]}
                      onPress={() => rsvp(item.id, status)}
                    >
                      <Text style={[st.rsvpBtnText, item.myRsvp === status && { color: c.accent, fontWeight: "600" }]}>{RSVP_LABELS[status]}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              )}
        ...
        ListEmptyComponent={
          <View style={{ alignItems: "center", marginTop: 80 }}>
            ...
            <TouchableOpacity style={[st.createBtn, { backgroundColor: c.accent, marginTop: 16 }]} onPress={() => setModalVisible(true)}>
              <Plus size={16} color="#FFFFFF" />
        ...
        <View style={[st.overlay, { backgroundColor: c.overlay }]}>
          <View style={[st.modal, { backgroundColor: c.surface, borderColor: c.border }]}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              ...
              <TouchableOpacity onPress={() => setModalVisible(false)}><X size={20} color={c.textSecondary} /></TouchableOpacity>
            </View>
            <TextInput
            ...
            />
            <TextInput
              style={[st.modalInput, st.modalTextArea, { backgroundColor: c.inputBg, color: c.text, borderColor: c.border }]}
              ...
              multiline
            />
            <TouchableOpacity style={[st.modalInput, { backgroundColor: c.inputBg, borderColor: c.border, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }]} onPress={() => setShowConvPicker(true)}>
            ...
            <TouchableOpacity style={[st.modalInput, { backgroundColor: c.inputBg, borderColor: c.border, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }]} onPress={() => setShowStartPicker(true)}>
              <Calendar size={16} color={c.accent} />
              <Text style={{ color: c.text, fontSize: 15 }}>
                {startsAt.toLocaleDateString()} {startsAt.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity style={[st.modalInput, { backgroundColor: c.inputBg, borderColor: c.border, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }]} onPress={() => setShowEndPicker(true)}>
              <Clock size={16} color={c.textMuted} />
              <Text style={{ color: endsAt ? c.text : c.textMuted, fontSize: 15 }}>
                {endsAt ? `End: ${endsAt.toLocaleDateString()} ${endsAt.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : "End time (optional)"}
              </Text>
            </TouchableOpacity>
            {endsAt && <TouchableOpacity onPress={() => setEndsAt(null)}><Text style={{ color: c.danger, fontSize: 13, textAlign: "center", marginBottom: 8 }}>Clear end time</Text></TouchableOpacity>}
            {showStartPicker && (
              <DateTimePicker
                value={startsAt}
                mode="datetime"
                display={Platform.OS === "ios" ? "spinner" : "default"}
                onChange={(_: any, date?: Date) => { setShowStartPicker(false); if (date) setStartsAt(date) }}
              />
            )}
            {showEndPicker && (
              <DateTimePicker
                ...
                mode="datetime"
                display={Platform.OS === "ios" ? "spinner" : "default"}
                onChange={(_: any, date?: Date) => { setShowEndPicker(false); if (date) setEndsAt(date) }}
              />
            )}
            {showConvPicker && (
              <View style={{ maxHeight: 200, marginBottom: 12 }}>
                <FlatList
                  ...
                  keyExtractor={(c) => c.id}
                  renderItem={({ item }) => (
                    <TouchableOpacity style={{ paddingVertical: 8, flexDirection: "row", alignItems: "center", gap: 8 }} onPress={() => { setConvId(item.id); setShowConvPicker(false) }}>
                      <Text style={{ color: item.id === convId ? c.accent : c.text, fontSize: 14, flex: 1 }}>{item.name || item.type}</Text>
            ...
            <View style={st.modalActions}>
              <TouchableOpacity onPress={() => setModalVisible(false)} style={{ paddingHorizontal: 20, paddingVertical: 10 }}>
              ...
              <TouchableOpacity
                ...
                disabled={creating}
              >
                {creating ? <ActivityIndicator size="small" color="#FFFFFF" /> : <Text style={st.confirmText}>Create Event</Text>}
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const st = StyleSheet.create({
  ...
  createBtn: { borderRadius: 20, paddingHorizontal: 14, paddingVertical: 8, flexDirection: "row", alignItems: "center", gap: 4 },
