...
import { View, Text, FlatList, TouchableOpacity, StyleSheet, RefreshControl, TextInput, Modal, ActivityIndicator, Image } from "react-native"
...
interface Conv {
  ...
  otherUser?: { id: string; username: string; displayName?: string; avatar?: string; status?: string; customStatus?: string }
  unreadCount?: number
}
...
export function ConversationsScreen({ onSelect }: { onSelect: (id: string) => void }) {
  ...
  useEffect(() => {
    if (userSearchQuery.length < 2) { setUserResults([]); return }
        ...
        setConvs((prev) =>
          prev.map((c) =>
            c.id === data.conversationId
              ? { ...c, lastMessage: { content: data.content, createdAt: data.createdAt, sender: { username: data.sender?.username || "" } } }
              : c
          ).sort((a, b) => {
          ...
          <TouchableOpacity style={s.item} onPress={() => onSelect(item.id)}>
            <View style={s.avatarWrap}>
              {item.avatar ? (
                ...
                <View style={s.avatar}>
                  <Text style={s.avatarText}>{item.type === "dm" ? (item.otherUser?.displayName?.[0] ?? item.otherUser?.username?.[0] ?? "?") : item.name?.[0] ?? "G"}</Text>
                </View>
              )}
              {item.type === "dm" && item.otherUser?.status && item.otherUser.status !== "offline" && (() => {
                const statusColors: Record<string, string> = { online: "#22C55E", away: "#EAB308", busy: "#EF4444" }
                return <View style={[s.statusDot, { backgroundColor: statusColors[item.otherUser!.status] || "#8888A0" }]} />
              })()}
            </View>
            <View style={s.itemContent}>
              <View style={s.itemTop}>
                <Text style={s.name} numberOfLines={1}>{item.name || (item.type === "dm" ? (item.otherUser?.displayName ?? item.otherUser?.username ?? "User") : item.type === "group" ? "Group" : "User")}</Text>
                {item.lastMessage && (
                  <Text style={s.time}>{formatTime(item.lastMessage.createdAt)}</Text>
                )}
              </View>
        ...
        <View style={s.modalContainer}>
          <View style={s.modalContent}>
            <View style={s.modalHeader}>
              ...
              <TouchableOpacity onPress={() => { setShowNewDM(false); setUserSearchQuery(""); setUserResults([]) }}>
...
const s = StyleSheet.create({
  ...
  addBtn: { width: 36, height: 36, borderRadius: 18, backgroundColor: "#181825", justifyContent: "center", alignItems: "center", borderWidth: 1, borderColor: "#252538" },
  logoutBtn: { padding: 4 },
  logout: { color: "#EF4444", fontSize: 14, fontWeight: "500" },
  searchRow: { flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: 16, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: "#252538", backgroundColor: "#0A0A0F" },
  ...
  avatar: { width: 48, height: 48, borderRadius: 24, backgroundColor: "#181825", justifyContent: "center", alignItems: "center", borderWidth: 1, borderColor: "#252538" },
  avatarText: { color: "#E8E8F0", fontSize: 18, fontWeight: "600" },
  statusDot: { position: "absolute", bottom: 0, right: 0, width: 12, height: 12, borderRadius: 6, borderWidth: 2, borderColor: "#0A0A0F" },
  itemContent: { flex: 1 },
  ...
  unreadBadge: { minWidth: 20, height: 20, borderRadius: 10, backgroundColor: "#6C8CFF", justifyContent: "center", alignItems: "center", paddingHorizontal: 6, marginLeft: 8 },
  ...
  modalContent: { backgroundColor: "#0A0A0F", borderTopLeftRadius: 20, borderTopRightRadius: 20, maxHeight: "80%", paddingBottom: 40, borderTopWidth: 1, borderTopColor: "#252538" },
  modalHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 20, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: "#252538" },
  modalTitle: { color: "#E8E8F0", fontSize: 18, fontWeight: "600" },
  userSearchRow: { flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: 16, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: "#252538" },
  userSearchInput: { flex: 1, color: "#E8E8F0", fontSize: 14, padding: 4 },
  userItem: { flexDirection: "row", alignItems: "center", padding: 14, borderBottomWidth: 1, borderBottomColor: "#252538" },
  userAvatar: { width: 40, height: 40, borderRadius: 20, backgroundColor: "#181825", justifyContent: "center", alignItems: "center", marginRight: 12, borderWidth: 1, borderColor: "#252538" },
