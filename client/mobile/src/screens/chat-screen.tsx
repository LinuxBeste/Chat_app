...
interface Msg { id: string; content: string; senderId: string; createdAt: string; sender: { username: string } }

export function ChatScreen({ conversationId, userId, onBack }: { conversationId: string; userId: string; onBack: () => void }) {
  const [messages, setMessages] = useState<Msg[]>([])
  const [input, setInput] = useState("")
  const flatRef = useRef<FlatList>(null)

  useEffect(() => {
    api<Msg[]>(`/api/conversations/${conversationId}/messages`).then(setMessages).catch(() => {})
  }, [conversationId])
...
const s = StyleSheet.create({
  ...
  header: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: "#2A2F3A" },
  ...
  input: { flex: 1, height: 40, backgroundColor: "#181B22", borderRadius: 20, paddingHorizontal: 16, color: "#F0F0F0", fontSize: 14, borderWidth: 1, borderColor: "#2A2F3A" },
  sendBtn: { marginLeft: 8, backgroundColor: "#4850BB", borderRadius: 20, paddingHorizontal: 20, height: 40, justifyContent: "center" },
