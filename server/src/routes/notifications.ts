...
router.get("/unread-count", authGuard, catchAsync(async (req: Request, res: Response) => {
...
router.post("/:id/read", authGuard, catchAsync(async (req: Request, res: Response) => {
  await db
...
router.post("/read-all", authGuard, catchAsync(async (req: Request, res: Response) => {
  await db
    .update(notifications)
    .set({ isRead: "true" })
    .where(and(eq(notifications.userId, req.user!.userId), eq(notifications.isRead, "false")))
