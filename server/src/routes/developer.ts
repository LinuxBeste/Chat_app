...
router.get("/webhooks", authGuard, catchAsync(async (_req: Request, res: Response) => {
  const list = await db.select().from(webhooks).where(eq(webhooks.userId, _req.user!.userId))
  res.json(list)
}))

router.post("/webhooks", authGuard, validate(webhookSchema), catchAsync(async (req: Request, res: Response) => {
  const wh = await db
    .insert(webhooks)
...
router.delete("/webhooks/:id", authGuard, catchAsync(async (req: Request, res: Response) => {
...
router.post("/api-keys", authGuard, catchAsync(async (req: Request, res: Response) => {
...
router.get("/api-keys", authGuard, catchAsync(async (req: Request, res: Response) => {
  const keys = await db
...
router.delete("/api-keys/:id", authGuard, catchAsync(async (req: Request, res: Response) => {
  await db
